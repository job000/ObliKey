...
export default function SupplierModal({ isOpen, onClose, onSuccess }: SupplierModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        alert('Kunne ikke opprette leverandør: ' + (response.error || 'Ukjent feil'));
      }
    } catch (error: any) {
      console.error('Error creating supplier:', error);
      const errorMessage = error.response?.data?.error || error.message || 'Ukjent feil';
      alert('Kunne ikke opprette leverandør: ' + errorMessage);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block w-full max-w-2xl my-8 overflow-hidden text-left align-middle transition-all transform bg-white rounded-lg shadow-xl">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h3 className="text-lg font-medium text-gray-900">Ny leverandør</h3>
